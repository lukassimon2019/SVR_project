...
message_content <- readLines("C:\\Users\\P-Simon\\Documents\\SVR\\prompts\\prompt_structured_output.txt")

# Set the path to the directory containing your PDF files
pdf_directory <- "C:\\Users\\P-Simon\\Documents\\SVR\\split_pdfs\\Inhaltsverzeichnis\\" #Change this to your directory
...
inhaltsverzeichnis_schema<-tidyllm_schema(
  kapitel=field_object(
    ...
    zweite_ebene=field_object(
      ...
      dritte_ebene=field_object(
        ueberschrift_drei=field_chr("Überschrift der dritten Ebene"),
...
for (pdf in pdf_filenames) {
  # Create the conversation object directly
  conversation <- llm_message(message_content, .pdf = paste0(pdf_directory, pdf)) |>
    chat(gemini(.model = "gemini-2.0-flash"), .json_schema = inhaltsverzeichnis_schema)
  
  # Store the conversation object in the list, using the PDF filename (without extension) as the key
  conversatioconvn_name <- gsub("\\.pdf$", "", pdf)
  conversations[[conversation_name]] <- conversation
}

# Save the list to a file
saveRDS(conversations, file = "C:\\Users\\P-Simon\\Documents\\SVR\\temp_store\\inhaltsverz_json_strings.rds")
...
parse_toc_json <- function(json_string) {
  ...
  for (kap in data_list$kapitel) {
    ...
    rows_list[[length(rows_list) + 1]] <- data.frame(
      ...
      Ueberschrift_Drei = NA_character_,
      Seite = s1,
      stringsAsFactors = FALSE
    )
      ...
      for (ebene2 in kap$zweite_ebene) {
        ...
        if (!is.null(ebene2$dritte_ebene) && length(ebene2$dritte_ebene) > 0) {
          for (ebene3 in ebene2$dritte_ebene) {
            ue3 <- ebene3$ueberschrift_drei %||% NA_character_
