...
txt_directory <- paste0(wd,"\\output\\inhaltsverzeichnis\\txt\\batch_to_be_processed\\")
...
txt_path_list <- list.files(txt_directory, pattern = "\\.txt$", full.names = TRUE)

#only store pdf names
txt_filenames <- lapply(txt_path_list, basename)
print(paste("These are the txt filenames",names(txt_filenames)))
...
for (txt in txt_filenames) {
  
  # Create conversation name by removing .pdf extension
  conversation_name <- gsub("\\.txt$", "", txt)
  
  # Initialize pdf_parts list for this conversation
  pdf_parts <- list()
  
  # Upload the PDF to Gemini
  pdf_content <- readLines(paste0(txt_directory,txt))
  single_string_content <- paste(pdf_content, collapse = "\n") 
  ...
  repeat {
    ...
    message<-paste("Here the pdf:",single_string_content,prompt_fin,"")
    
    
    # Process with Gemini API
    conversation_part <- llm_message(message) |>
      chat(gemini(
    ...
    writeLines(message, paste0(output_directory, conversation_part_name, "_message_fin.txt"))
