...
prompt_vec <- readLines(paste0(prompt_directory,"pdf_to_json_split_prompt_3.txt"))
...
for (pdf in pdf_filenames) {
  ...
  repeat {
    ...
    conversation_part <- llm_message(prompt_fin) |>
      ...
      ), .json_schema = inhaltsverzeichnis_schema)
    ...
    if (grepl("end_json", conversation_part_content)) {
      # Remove the end_json marker
      pdf_parts[[run_number]] <- gsub("end_json", "", conversation_part_content)
      break
    }
  ...
  all_contents<-loadRDS(all_contents, file = paste0(temp_directory, "inhaltsverz_json_strings_split.rds"))
